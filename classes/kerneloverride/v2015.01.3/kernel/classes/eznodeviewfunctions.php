...
class eZNodeviewfunctions
{
    ...
    static function generateViewCacheFile( $user, $nodeID, $offset, $layout, $language, $viewMode, $viewParameters = false, $cachedViewPreferences = false, $viewCacheTweak = '' )
    {
        ...
        // Note: Here we alter content cache storage to be ini [RegionalSettings] Locale specific to support on the fly changing of locale without having to clear content caches on the fly or disable them entirely.
        $currentSiteAccess = $currentSiteAccess . '_' . $ini->variable( 'RegionalSettings', 'Locale' );
