...
class ezpKernelWeb implements ezpWebBasedKernelHandler
{
    ...
    public function __construct( array $settings = array() )
    {
        ...
        // BEGIN BC : BCSwitchAdminLanguage : Kernel Hack : Alter ini settings (siteaccess language and cache) to change the admin UI language on the fly
        // make sure we get a new $ini instance now that it has been reset
        $ini = eZINI::instance();
        $http = eZHTTPTool::instance();

        // Fetch BCSwitchAdminLanguage extension settings
        $bcSwitchAdminLanguageINI = eZINI::instance( 'bcswitchadminlanguage.ini' );
        $BCSwitchAdminLanguageLocaleIdentifierSessionVariableName = $bcSwitchAdminLanguageINI->variable( 'BCSwitchAdminLanguageSettings', 'SwitchAdminLanguageLocaleIdentifierSessionVariableName' );
        $sharedAdminSiteAccess = $bcSwitchAdminLanguageINI->variable( 'BCSwitchAdminLanguageSettings', 'SharedAdminSiteaccessName' );

        if( $GLOBALS['eZCurrentAccess']['name'] === $sharedAdminSiteAccess
            && $http->hasSessionVariable( $BCSwitchAdminLanguageLocaleIdentifierSessionVariableName ) )
        {
            $switchLanguageLanguageLocaleIdentifier = $http->sessionVariable( $BCSwitchAdminLanguageLocaleIdentifierSessionVariableName );
            $ini->setVariable( 'RegionalSettings', 'Locale', $switchLanguageLanguageLocaleIdentifier );
        }
        // END BC : BCSwitchAdminLanguage : Kernel Hack : Alter ini settings (siteaccess language and cache) to change the admin UI language on the fly
